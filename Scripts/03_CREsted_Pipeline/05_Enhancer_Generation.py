...
def cosine_similarty_optimizer(
...
Optimizer_function = crested.tl.design.EnhancerOptimizer(optimize_func = cosine_similarty_optimizer)
...
for i in range(len(met_hi_designed_sequences)):
        ...
        crested.pl.region.bar(prediction, classes=list(adata_specific.obs_names), title=f"Designed enhancer {i+1}", ax=ax, show=False)
        plt.savefig(f"output/CREsted_ISE/Cos/MH_Designed_enhancer_{i+1}.png")
        plt.close()
...
for i in range(len(met_lo_designed_sequences)):
        ...
        crested.pl.region.bar(prediction, classes=list(adata_specific.obs_names), title=f"Designed enhancer {i+1}", ax=ax, show=False)
        plt.savefig(f"output/CREsted_ISE/Cos/ML_Designed_enhancer_{i+1}.png")
        plt.close()
