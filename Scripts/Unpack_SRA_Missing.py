...
def main():
    ...
    all_sra_files = sorted(list(Path(base_dir).rglob("*.sra")))
    
    # Filter only for the target missing files
    sra_files = [f for f in all_sra_files if f.stem in TARGET_SRRS]
    
    if not sra_files:
        print("No target .sra files found.")
        sys.exit(0)
